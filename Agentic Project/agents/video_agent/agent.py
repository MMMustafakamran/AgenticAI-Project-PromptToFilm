        ...
        for index, scene in enumerate(state.scenes):
            ...
            try:
                if use_video:
                    ...
                    existing = scene.image_path and Path(scene.image_path).exists()
                    if existing:
                        if progress_cb: progress_cb(int((index / total_scenes) * 70), f"Reusing existing video for {scene.scene_id}")
                    else:
                    ...
                    existing = scene.image_path and Path(scene.image_path).exists()
                    if existing:
                        if progress_cb: progress_cb(int((index / total_scenes) * 70), f"Reusing existing image for {scene.scene_id}")
                    else:
